...
function HomePage() {
  return <div className="page-reveal max-w-full overflow-x-clip">
    ...
    <section className="relative overflow-hidden border-y border-border bg-secondary px-6 py-24 text-foreground sm:px-10 lg:py-28">
...
function HomeCompare({ label, loose, livoara }: { label: string; loose: string; livoara: string }) { return <article className="border border-border"><h3 className="bg-muted/60 px-4 py-3 text-sm font-medium">{label}</h3><dl className="grid grid-cols-2 divide-x divide-border"><div className="p-4"><dt className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Loose vanity</dt><dd className="mt-2 text-xs">{loose}</dd></div><div className="bg-secondary/55 p-4"><dt className="text-[10px] uppercase tracking-[0.12em] text-accent">LIVOARA</dt><dd className="mt-2 text-xs font-medium">{livoara}</dd></div></dl></article>; }
