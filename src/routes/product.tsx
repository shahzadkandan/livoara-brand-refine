...
import { Check, ChevronDown, Gift, Minus, Plus, ShieldCheck, Sparkles, Truck } from "lucide-react";
...
function ProductPage() {
  ...
  return <div className="page-reveal">
    <div className="border-b border-border bg-muted/40 px-5 py-3 text-center text-xs uppercase tracking-[0.16em] text-muted-foreground">Home / The LIVOARA Travel Vanity</div>
    <div className="mx-auto grid max-w-[1450px] gap-10 px-5 py-8 sm:px-8 lg:grid-cols-[1.08fr_.92fr] lg:gap-16 lg:px-12 lg:py-16">
      <div><div className="aspect-square overflow-hidden bg-secondary"><img src={heroAsset.url} alt="LIVOARA Travel Vanity" className={`h-full w-full scale-[1.6] object-cover ${crops[selected]}`} /></div><div className="mt-3 grid grid-cols-3 gap-3">{crops.map((crop, index) => <Button variant="ghost" key={crop} onClick={() => setSelected(index)} className={`aspect-[4/3] h-auto overflow-hidden border bg-secondary p-0 ${selected === index ? "border-foreground" : "border-border"}`} aria-label={`View product image ${index + 1}`}><img src={heroAsset.url} alt="" className={`h-full w-full scale-[2.5] object-cover ${crop}`} /></Button>)}</div></div>
      <div className="lg:sticky lg:top-28 lg:self-start"><div className="flex items-center gap-3 text-sm"><span className="text-accent">★ ★ ★ ★ ★</span><span className="text-muted-foreground">[Rating & review count]</span></div><h1 className="mt-4 font-display text-5xl leading-tight sm:text-6xl">The LIVOARA Travel Vanity</h1><p className="mt-5 leading-8 text-muted-foreground">A luminous vanity, thoughtful organizer, and travel companion—beautifully designed as one.</p>
        <div className="mt-7 grid grid-cols-2 gap-3"><BenefitCard icon={<Sparkles />} text="Illuminated mirror" /><BenefitCard icon={<Check />} text="Organized storage" /><BenefitCard icon={<Truck />} text="Travel-ready design" /><BenefitCard icon={<Gift />} text="Thoughtful gifting" /></div>
        <div className="mt-8 flex items-end gap-4 border-y border-border py-5"><div><p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Price</p><p className="mt-2 text-3xl font-medium">[Price]</p></div><p className="pb-1 text-sm text-muted-foreground">[Add any verified offer]</p></div>
        <ul className="mt-7 space-y-4">{benefits.map((benefit) => <li key={benefit} className="flex gap-3 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-accent" />{benefit}</li>)}</ul>
...
function BenefitCard({ icon, text }: { icon: ReactNode; text: string }) { return <div className="flex min-h-20 items-center gap-3 border border-border bg-muted/50 px-4 text-sm font-medium"><span className="text-accent [&>svg]:size-5">{icon}</span><span>{text}</span></div>; }