...
function ProductPage() {
  ...
  return <div className="page-reveal">
    <div className="mx-auto grid max-w-[1450px] gap-10 px-5 py-8 sm:px-8 lg:grid-cols-[1.18fr_.82fr] lg:gap-16 lg:px-12 lg:py-16">
      <div><div className="aspect-[5/4] overflow-hidden bg-secondary"><img src={heroAsset.url} alt="LIVOARA Travel Vanity" className={`h-full w-full scale-[1.35] object-cover ${crops[selected]}`} /></div><div className="mt-3 grid grid-cols-3 gap-3">{crops.map((crop, index) => <button key={crop} onClick={() => setSelected(index)} className={`aspect-square overflow-hidden border bg-secondary ${selected === index ? "border-foreground" : "border-border"}`} aria-label={`View product image ${index + 1}`}><img src={heroAsset.url} alt="" className={`h-full w-full scale-[2.5] object-cover ${crop}`} /></button>)}</div></div>
